...
from src.app.agents.specialized_nodes import reviewer_agent, searcher_agent, writer_agent


@pytest.mark.asyncio
async def test_searcher_agent_no_knowledge(mock_llm):
    """测试 Searcher Agent: 不需要检索的情况"""
    state = {"messages": [HumanMessage(content="你好")], "knowledge_context": "", "iteration": 0}
    # Mock LLM 返回 NO
