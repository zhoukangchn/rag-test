...
async def analyze_correlation_node(state: DiagnoserState) -> dict[str, Any]:
    ...
    logger.info("[Diagnoser] 正在分析指标与日志的关联性...")
...
async def generate_hypothesis_node(state: DiagnoserState) -> dict[str, Any]:
    """根据分析结果生成更精确的根因假设"""
    logger.info("[Diagnoser] 正在生成根因假设...")
    ...
    if "I/O 等待" in reflection:
        hypotheses.append(
            {
                "hypothesis": "磁盘或数据库 I/O 阻塞",
                "confidence": 0.9,
                "evidence": ["High cpu_iowait"],
            }
        )
    elif "内核态" in reflection:
        hypotheses.append(
            {
                "hypothesis": "系统资源争抢或内核瓶颈",
                "confidence": 0.8,
                "evidence": ["High cpu_system"],
            }
        )
    elif "用户态" in reflection:
        hypotheses.append(
            {
                "hypothesis": "业务逻辑死循环或算力不足",
                "confidence": 0.85,
                "evidence": ["High user-space CPU consumption"],
            }
        )
    else:
        # 兜底
        hypotheses = [
            {
                "hypothesis": "数据库连接池配置过小",
                "confidence": 0.8,
                "evidence": ["Connection pool exhausted logs"],
            },
            {
                "hypothesis": "下游服务响应慢导致连接积压",
                "confidence": 0.5,
                "evidence": ["Increased request latency"],
            },
        ]
