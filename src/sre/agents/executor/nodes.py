...
from src.sre.agents.shared.state import ActionType, ExecutorState
        ...
        if tool == "restart_pod":
            ...
            if kind == "deployment":
                deploy = await Deployment.get(
                    name, namespace=params.get("namespace", "default"), api=api
                )
...
async def execute_tool_node(state: ExecutorState) -> dict[str, Any]:
    ...
    return {"executed_actions": executed_results}


async def verify_result_node(state: ExecutorState) -> dict[str, Any]:
    """验证执行结果"""
    logger.info("[Executor] 正在验证执行结果...")
